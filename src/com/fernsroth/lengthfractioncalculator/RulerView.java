...
public class RulerView extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		for (float in = leftInchesX;; in += 1.0f / 128.0f) {
			...
			if (isRound(in)) {
				...
				CanvasUtils.drawHvAlignedText(canvas, x + 4, centerY - minor1TickHeight, String.format("%d", (int) in),
						tickTextPaint, Paint.Align.LEFT, CanvasUtils.TextVertAlign.Top);
		...
		for (float mm = leftMillimetersX;; mm += 1.0f) {
			...
			if (isRound(mm / 10.0f)) {
				...
				CanvasUtils.drawHvAlignedText(canvas, x + 4, centerY + minorCentimeterTickHeight,
						String.format("%dmm", (int) mm), tickTextPaint, Paint.Align.LEFT,
						CanvasUtils.TextVertAlign.Baseline);
			} else {
				canvas.drawLine(x, centerY + 2, x, centerY + minorMillimeterTickHeight, tickPaint);
			}
		}

		canvas.drawLine(centerX, 0, centerX, canvas.getHeight(), centerLinePaint);
	}
	...
	public float getMillimeters() {
		return getInches() * 25.4f;
	}

	public float getMillimeters(int x) {
		return getInches(x) * 25.4f;
	}
